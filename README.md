...
点击进入 → `WASD` 移动、`←→` 转向，全程不用碰鼠标 →
或者**直接点地面走过去**（这时镜头会自动朝向路径）→ 光标移到发光的空椅子上，
椅子上方会浮出提示 → `E` 或直接点击坐下 → `Q` 起身回到大厅。

仍然可以按住拖拽手动环视。**手感参数全部在 leva 的「手感」里可调**：
移动速度、转向速度、头部晃动（晕 3D 可以直接拉到 0），
以及切换成老式坦克操作（A/D 转向、Q/E 侧移）。
...
**A/D 转向而不是平移，因为"准"来自没有过渡期**

"按住鼠标才能转"在第一人称里走远路很累，所以要让键盘能独立控制方向。

第一版做成了「WASD 是世界方向 + 镜头追着移动方向转」，结果**方向感很不准**。
问题出在过渡期：朝北站着按 D，人立刻往东走，镜头却要半秒才转过来 ——
那半秒里按键方向和屏幕上的"前"对不上。而且第一人称里根本没有"北"的视觉锚点，
世界方向本来就是等距视角游戏的方案。

于是改成了坦克式：`A/D` 转向、`W/S` 沿朝向前后。方向确实准了（实测偏差 0.0000 rad），
但**丢掉侧移之后又难受了** —— 在满是桌子的厅里挪半米要"转身-走-再转回来"。

最终落到：`WASD` 保持最熟悉的那套（W/S 前后、A/D 侧移，都相对镜头），
**转向挪到方向键 `←→`**。这样键盘依然能独立控制方向、不用碰鼠标，
同时侧移还在。坦克模式保留成一个选项。

手感这种东西猜不出来，所以速度、转向、晃动全做成了滑块。
...
**点地面走过去**没有寻路，就是朝目标直走，撞到东西靠碰撞滑开；
一个开阔的厅里够用。判断是不是"点击"用的是 R3F 事件里的 `e.delta`
（按下到抬起之间移动的像素数），超过 4px 算拖拽，不下达移动指令。

两个坑：

- **点在桌子上是很自然的操作，但桌心走不到。** 所以设目标时先用
  `nudgeOutOfObstacles` 把它推到障碍物外，语义就变成"走到那张桌子旁边"。
- **卡住检测必须看实际位移，不能看速度。** 顶着桌子推的时候速度一直是满的，
  只是每帧都被碰撞推回原处 —— 按速度判断永远判不出卡住，玩家会贴着桌子一直抖。
