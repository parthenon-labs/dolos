...
它的职责是让 prompt 构造、结构化输出解析、非法动作重试、防泄漏审查、
发言竞价这整条链路在没有 API key 时也能端到端跑通。
**分清"管线坏了"和"模型不行"，是这类项目里最省时间的一件事。**

实测 300 局：2609 局/秒，64 次非法动作全部被重试和兜底吸收，没有一局崩掉 ——
LLM 给非法动作是常态而不是异常，这条路径必须在接真模型之前就验过。

**发言用竞价决定顺序。** 抄自 Google Research 的 Werewolf Arena：
每人报一个 0-4 的发言意愿（0 旁听 / 4 被点名必须回应），最高者发言，
平手时上一轮被点名的人优先。固定轮流发言在阿瓦隆里很失真 —— 真实讨论是抢话的。
这套现在跑在文字上，**接语音时可以原样复用：竞价就是抢麦**。

**发言前有一道泄漏审查。** AvalonBench 记录过一个结构性失效：
LLM 坏人经常在讨论里自曝身份。那不是能力问题 —— system prompt 里明写着
"你是刺客，同伙是 3 号"，模型的注意力就会往那儿泄，靠把提示词写得更严厉压不住。

所以不劝，直接审查：把私有信息和发言做对照，命中就打回。纯本地字符串规则，
不额外花一次 API 调用。实测拦截样例：

```
3 号想说「我确定 1 号是坏人，因为我看得到坏人是谁。」
  → 拦截：坏人以确定语气点名了同伙 1
```

同一句话 2 号（派西维尔）说就**没有**被拦 —— 对坏人那是交底，对他那是诈唬。
**撒谎是合法战术，泄漏不是**，这条线必须划准。
