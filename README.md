...
点击进入 → `W/S` 前后、`A/D` 转向、`Q/E` 侧移，全程不用碰鼠标 →
或者**直接点地面走过去**（这时镜头会自动朝向路径）→ 光标移到发光的空椅子上，
椅子上方会浮出提示 → `E` 或直接点击坐下 → `Q` 起身回到大厅。

仍然可以按住拖拽手动环视。leva 的「操作 → 操作方式」可以切回
「WASD 平移 · 拖拽转视角」，转向速度也能调。
...
现在改成 `A/D` 直接转朝向、`W/S` 沿朝向前后。**W 永远精确等于你正看着的方向，
中间没有任何过渡期**，这才是"准"。实测在任意角度下移动方向与朝向偏差 0.0000 rad。
