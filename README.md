...
点击进入 → `WASD` 走动，或者**直接点地面走过去** → **镜头自动转向你正在走的方向**，
不需要按住鼠标 → 光标移到发光的空椅子上，椅子上方会浮出提示 → `E` 或直接点击坐下，
镜头会推进并收拢到牌桌视角 → `Q` 起身回到大厅。

仍然可以按住拖拽手动环视（站着不动看东西时有用）。leva 的「操作 → 自动转向」
可以关掉，退回到原来的手动模式。
...
**镜头自动朝向移动方向，WASD 因此必须是世界方向**

"按住鼠标才能转"在第一人称里走远路很累。现在镜头会自己平滑转到你正在走的方向。

这里有个**必然的连带后果**：WASD 必须变成世界方向（W 永远是往 -Z 走），
不能再是相对镜头的。因为如果 A 表示"相对镜头往左"，而镜头又要转去朝向移动方向，
两者会互相追着跑，人会原地打转。**世界方向 + 镜头跟随，是唯一稳定的组合。**
关掉自动转向时，WASD 自动切回相对镜头。

朝向只在速度超过阈值时更新 —— 速度趋近 0 时方向向量会剧烈抖动，
不设阈值的话每次停步镜头都会甩一下。

**点地面走过去**没有寻路，就是朝目标直走，撞到东西靠碰撞滑开；
一个开阔的厅里够用。贴着墙蹭 45 帧还没进展就自动放弃，免得玩家一直顶着墙。
判断是不是"点击"用的是 R3F 事件里的 `e.delta`（按下到抬起之间移动的像素数），
超过 4px 算拖拽，不下达移动指令。
